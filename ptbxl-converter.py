...
Versão Corrigida para Windows e nomenclatura _lr
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ptbxl_conversion.log', encoding='utf-8'),
...
class PTBXLConverter:
    ...
    def extract_ecg_id(self, filename: str) -> Optional[int]:
        """
        Extrai o ID do ECG do nome do arquivo
        
        Args:
            filename: Nome do arquivo (ex: "00001_lr", "00001", etc)
            
        Returns:
            ID do ECG ou None se não conseguir extrair
        """
        # Tentar diferentes padrões
        patterns = [
            r'^(\d+)_lr$',    # Padrão com _lr
            r'^(\d+)_hr$',    # Padrão com _hr
            r'^(\d+)$',       # Apenas números
        ]
        ...
        # Se não conseguir, tentar extrair qualquer sequência de dígitos no início
        match = re.match(r'^(\d+)', filename)
        if match:
            return int(match.group(1))
        
        logger.warning(f"Não foi possível extrair ID de: {filename}")
        return None
    ...
    def validate_environment(self) -> bool:
        ...
        logger.info(f"[OK] Ambiente validado")
        logger.info(f"[OK] Encontrados {self.stats['total_records']} registros")
        
        # Verificar alguns exemplos de nomenclatura
        if hea_files:
            sample_files = hea_files[:5]
            logger.info("Exemplos de arquivos encontrados:")
    ...
    def load_metadata(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"[OK] Database carregado: {len(df_db)} registros")
        
        # Carregar códigos SCP
        scp_path = self.base_path / 'scp_statements.csv'
        df_scp = pd.read_csv(scp_path, index_col=0)
        logger.info(f"[OK] SCP codes carregados: {len(df_scp)} códigos")
    ...
    def process_labels(self, df_db: pd.DataFrame, df_scp: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"[OK] Identificadas {n_conditions} condições únicas")
        ...
        for patient_dir in sorted(self.records_path.iterdir()):
            if patient_dir.is_dir():
                for record_file in patient_dir.glob("*.hea"):
                    ...
                    ecg_id = self.extract_ecg_id(record_file.stem)
                    if ecg_id is not None:
                        all_records.append((ecg_id, record_path))
        
        logger.info(f"[OK] Encontrados {len(all_records)} registros válidos para processar")
    ...
    def _save_final_results(self, X, Y_single, Y_multi, ecg_ids, 
        ...
        logger.info(f"[OK] X.npy salvo: {X.shape}")
        
        np.save(self.output_path / 'Y.npy', Y_single)
        logger.info(f"[OK] Y.npy salvo: {Y_single.shape}")
        
        np.save(self.output_path / 'Y_multilabel.npy', Y_multi)
        logger.info(f"[OK] Y_multilabel.npy salvo: {Y_multi.shape}")
        
        np.save(self.output_path / 'ecg_ids.npy', ecg_ids)
        logger.info(f"[OK] ecg_ids.npy salvo: {ecg_ids.shape}")
        ...
        logger.info("[OK] metadata.json salvo")
...
"""
        ...
        logger.info("[OK] dataset_info.txt salvo")
...
def main():
    ...
    try:
        ...
        print(f"[OK] Arquivos salvos em: {converter.output_path}")
